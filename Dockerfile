...
RUN apt -y install libkrb5-dev opencl-dev libpcap-dev libsane-dev libv4l-dev libgphoto2-dev libtiff-dev libpulse-dev libgstreamer1.0-dev libudev-dev libcapi20-dev libgstreamer-plugins-base1.0-dev
RUN apt -y install git libdbus-1-dev fcitx-libs-dev libsamplerate0-dev libibus-1.0-dev
RUN apt -y install mingw-w64

# Wine Staging
RUN apt -y install gtk3.0-dev libgtk-3-dev libva-dev
RUN apt -y install libavcodec-dev

# Vulkan
RUN apt -y install libvulkan-dev
